...
### 3-1. 근거 없는 어투 지적 금지
- '~해 주세요' 체는 한국어 UX 라이팅에서 이미 널리 쓰이는, 명령형(~하세요)보다 부드럽고 정중한 요청형이다. **양쪽 방향 모두** 지적하지 않는다: '~해 주세요'를 '비격식적'이라거나 '더 격식 있게(예: ~하시기 바랍니다)' 고치라고 하지 않고, 반대로 '~해 주세요'를 '불필요하게 딱딱하다'며 더 간결한 명령형(예: ~하세요)으로 바꾸라고도 하지 않는다 — '~하세요'가 '~해 주세요'보다 더 직접적인 명령형에 가깝다는 점을 기억한다. '~해 주세요'는 이미 적절한 정중함 수준의 기본값이다.
- 반대로 '~하세요'(명령형)를 '부드러운 표현이 필요하다'며 '~해 주세요'로 바꾸라는 지적은 **같은 문서 안에 실제로 더 정중하게 쓰인 대조 사례**가 있을 때만 보고한다 — 다른 근거 없이 명령형이라는 이유만으로 지적하지 않는다.
